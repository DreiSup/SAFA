...
import { useAudio } from '@/context/AudioContext'                              
  ...
  const NAV = [                                                                  
    ...
    { id: 'profile', label: 'Perfil',   icon: User,       to: null, enabled: false },                                                              
  ]

  const REPORT = {
    id: 'report', label: 'Reporte', to: '/report', enabled:true
  }
  ...
  function SafaMark() {                                 
    return (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        ...
        <line x1="12" y1="2" x2="12" y2="6" stroke="var(--s-accent)" strokeWidth="1.4" />                                                           
      </svg>
    )                                                                            
  }                                                     
    ...
    return (
      <aside style={{                                   
        ...
        <div style={{
          ...
          display: 'flex', justifyContent: expanded ? 'space-between' : 'center', alignItems: 'center',                                                         
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>       
            <SafaMark />                                                         
            {expanded && <span style={{ fontWeight: 600, letterSpacing: '0.14em', fontSize: 13, color: 'var(--s-fg)' }}>SAFA</span>}                            
          </div>                                        
          {expanded && (                                                         
            <button onClick={() => setExpanded(false)}
                style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--s-fg-3)', display: 'flex', padding: 4, borderRadius: 6 }}
            >
              <ChevronLeft size={16} />                                          
            </button>                                                            
          )}
        ...
        {expanded && (
          <div style={{ padding: '6px 18px 14px', fontSize: 10, color: 'var(--s-fg-3)', letterSpacing: '0.16em', textTransform: 'uppercase' }}>       
            Navegación
          </div>                                                                 
        )}                                                                     
          ...
          {NAV.map(({ id, label, icon: Icon, to, enabled }) => {
            ...
            const item = (                                                     
              <span style={{
                ...
                borderRadius: 8,                                                 
                color: !enabled ? 'var(--s-fg-3)' : active ? 'var(--s-fg)' : 'var(--s-fg-2)',                                                               
                ...
                {expanded && active && <span style={{ width: 4, height: 4, borderRadius: 99, background: 'var(--s-accent)', flexShrink: 0 }} />}          
                {expanded && !enabled && (                                       
                  <span style={{ fontSize: 9, color: 'var(--s-fg-3)', border: '1px solid var(--s-border-strong)', padding: '1px 5px', borderRadius: 4 }}>    
                    Pronto
                  </span>                                                        
                )}                                                             
              </span>
        ...
        <div 
            style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--s-bg-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: 'var(--s-fg-1)', flexShrink: 0, border: '1px solid var(--s-border-strong)' }}
        >                                
            A                                                                  
        </div>
          {expanded && (
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: 10, color: 'var(--s-fg-3)' }}>
                SAFA
              </div>   
            </div>                                                               
          )}
